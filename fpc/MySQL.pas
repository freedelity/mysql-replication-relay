...
uses
  ...
  SysUtils;
  {$ELSE}
  SysUtils, Windows;  // Needed for some type definitions
  {$ENDIF}
...
var
  ...
  mysql_set_character_set: function(_mysql: PMYSQL; _set: pChar): longword; {$IFDEF LINUX}cdecl;{$ELSE}stdcall;{$ENDIF}
...
//  myodbc_remove_escape: procedure(_mysql: PMYSQL; name: pChar); {$IFDEF LINUX}cdecl;{$ELSE}stdcall;{$ENDIF}
  ...
  mysql_library_init: function(argc: Integer; Argv: PPchar; Groups: PPchar): Integer; {$IFDEF LINUX}cdecl;{$ELSE}stdcall;{$ENDIF}
...
function libmysql_load(name: pChar): byte;
  ...
  procedure assign_proc(var proc: Pointer; name: pChar);
  begin
    {$IFDEF FPC}
    proc := GetProcAddress(libmysql_handle, name);
    {$ELSE}
    proc := GetProcAddress(Cardinal(libmysql_handle), name);
    {$ENDIF}
    if proc = nil then
    begin
      libmysql_status := LIBMYSQL_INCOMPATIBLE;
      AddToLog(llDebug, 'libmysql_load => Unable to find procedure '+name);
    end;
  end;
...
begin
  libmysql_free;
  {$IFDEF LINUX}
  if name = nil then name := 'libmysqlclient.so';
  {$ELSE}
  if name = nil then name := 'libmysql.dll';
  {$ENDIF}
  libmysql_handle := ClassCompat.LoadLibrary(name);

  {$IFDEF LINUX}

  {$IFDEF cpux86_64} 
  if libmysql_handle = nil then
  begin
    name := '/usr/lib64/libmysqlclient.so.12';
    libmysql_handle := ClassCompat.LoadLibrary(name);
  end;
  if libmysql_handle = nil then
  begin
  ...
  if libmysql_handle = nil then
  begin
    name := '/usr/lib64/mysql/libmysqlclient.so.12';
    libmysql_handle := ClassCompat.LoadLibrary(name);
  end;
  if libmysql_handle = nil then
  begin
    name := '/usr/lib64/mysql/libmysqlclient.so.15';
    libmysql_handle := ClassCompat.LoadLibrary(name);
  end;
  if libmysql_handle = nil then
  begin
    name := '/usr/lib64/mysql/libmysqlclient.so.18';
    libmysql_handle := ClassCompat.LoadLibrary(name);
  end;  
  {$ELSE}  
  if libmysql_handle = nil then
  begin
    name := '/usr/lib/libmysqlclient.so.12';
    libmysql_handle := ClassCompat.LoadLibrary(name);
  end;
  if libmysql_handle = nil then
  begin
    name := '/usr/lib/libmysqlclient.so.18';
    libmysql_handle := ClassCompat.LoadLibrary(name);
  end;
  if libmysql_handle = nil then
  begin
    name := '/usr/lib/mysql/libmysqlclient.so.12';
    libmysql_handle := ClassCompat.LoadLibrary(name);
  end;
  if libmysql_handle = nil then
  begin
    name := '/usr/lib/mysql/libmysqlclient.so.15';
    libmysql_handle := ClassCompat.LoadLibrary(name);
  end;
  if libmysql_handle = nil then
  begin
    name := '/usr/lib/mysql/libmysqlclient.so.18';
    libmysql_handle := ClassCompat.LoadLibrary(name);
  end;
  {$ENDIF}
  {$ENDIF}

  if libmysql_handle = nil then libmysql_status := LIBMYSQL_MISSING
  else begin
    ...
    {$ENDIF} // HAVE_OPENSSL
    //assign_proc(@mysql_connect, 'mysql_connect'); //Not anymore in 4.0
    ...
    //assign_proc(@mysql_create_db, 'mysql_create_db'); //Not anymore in 4.0
    //assign_proc(@mysql_drop_db, 'mysql_drop_db'); //Not anymore in 4.0
...
//    assign_proc(@myodbc_remove_escape, 'myodbc_remove_escape');
    ...
    //assign_proc(@mysql_library_init, 'mysql_library_init');
(*    {$IFDEF FPC}
    //mysql_library_init := GetProcAddress(libmysql_handle, 'mysql_library_init');
    {$ELSE}
    mysql_library_init := GetProcAddress(Cardinal(libmysql_handle), 'mysql_library_init');
    {$ENDIF}
    if @mysql_library_init = nil then
      assign_proc(@mysql_library_init, 'mysql_server_init');*)
  end;
  ...
  if result <> LIBMYSQL_READY then
    AddToLog(llDebug, 'libmysql_load => Unable to load MySQL Driver ('+IntToStr(Integer(result))+')');
end;

procedure libmysql_free;
begin
  if libmysql_handle <> nil then UnloadLibrary(libmysql_handle);
  libmysql_handle := nil;
